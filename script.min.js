...
(function () {
	...
	function toast(msg) {
		...
		el.innerText = msg;
		// Fixed positioning to avoid layout shift
		el.style.position = 'fixed';
		el.style.right = '12px';
		el.style.top = '12px';
		el.style.zIndex = '10000';
		el.style.background = '#230AC7';
		el.style.color = '#fff';
	...
	window.setOutputFieldValue = function () {
		...
		let items = [];
		try { 
			items = JSON.parse(sessionStorage.getItem(STORAGE_KEYS.symbols)) || []; 
		} catch (e) { 
			items = []; 
		}
		...
		const itemHeight = 42; // Approximate height of each symbol
		container.style.minHeight = (container.offsetHeight + (items.length * itemHeight)) + 'px';
		...
		runIdle(() => {
			batchDOMWrite(() => {
				...
				// Make symbols visible after layout is complete
				setTimeout(() => {
	...
	window.onPageLoad = function () {
		// Defer non-critical restore operation
		runIdle(function() {
			let snap = null;
			try { snap = JSON.parse(sessionStorage.getItem(STORAGE_KEYS.snapshot)) || null; } catch (e) { snap = null; }
			
			if (!snap || snap.page !== location.pathname) return;

			// Batch DOM updates
			batchDOMWrite(function() {
				const container = document.getElementById(IDS.symbolContainer);
				if (container && Array.isArray(snap.symbols) && snap.symbols.length) {
					...
					const frag = document.createDocumentFragment();
					for (let i = 0; i < snap.symbols.length; i++) {
						const txt = snap.symbols[i];
						const d = document.createElement('div');
						d.className = 'symbol';
						d.setAttribute('aria-hidden', 'true'); // Hide from screen readers until fully loaded
						d.style.visibility = 'hidden'; // Hide during render
						d.textContent = txt;
					...
					// Make visible after layout is complete
					requestAnimationFrame(() => {
						container.style.minHeight = '';
						Array.from(container.querySelectorAll('.symbol')).forEach(el => {
							el.style.visibility = '';
							el.removeAttribute('aria-hidden');
						});
					});
	...
	async function ensureDependencies() {
		// All external dependencies removed to reduce blocking time
		return;
	}
	...
	function runIdle(fn) {
		try {
			if (typeof requestIdleCallback === 'function') {
				requestIdleCallback(function () { try { fn(); } catch (e) {} }, { timeout: 1500 });
			} else {
	...
	function initInfiniteScroll() {
		// Infinite scroll disabled to prevent 404 errors
		return;
	}
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		setTextboxValue();
		// Defer heavier tasks to idle to reduce TBT
		runIdle(setOutputFieldValue);
		runIdle(onPageLoad);
		// Dependencies and infinite scroll removed for performance
	});
})();
